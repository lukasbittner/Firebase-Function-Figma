...
const API_LINK = `rest/api/2`

// The function is called to fetch ticket data from Jira
exports.getIssue = functions.https.onRequest(async(req, res) => {
    cors(req, res, () => {
        ...
        for (let i = 0; i < issueIds.length; i++) {
            const id = issueIds[i]
            const ticket_url = `https://${companyName}/${API_LINK}/issue/${id}`;
            const configData =
                axios({
                    method: 'get',
                    url: ticket_url,
...
// The function is called to post the Figma link to the Jira ticket
exports.postLinkToIssue = functions.https.onRequest(async(req, res) => {
    cors(req, res, () => {
        ...
        const remotelink_url = `https://${companyName}/${API_LINK}/issue/${issueId}/remotelink`;
        const configData = {
            method: 'post',
            url: remotelink_url,
            headers: {
...
// The function is called to test wether the user has entered the right authentication details.
exports.testAuthentication = functions.https.onRequest(async(req, res) => {
    cors(req, res, () => {
        const { basicAuth, companyName } = JSON.parse(req.body)

        const auth_url = `https://${companyName}/${API_LINK}/myself`;
        const configData = {
            method: 'get',
            url: auth_url,
            headers: {
